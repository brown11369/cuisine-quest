...
const OrderCancel = () => {
    ...
    useEffect(() => {
        if (session_id) {
            const cancelOrder = async () => {
                try {
                    const response = await fetch(`${PATCH_CHECKOUT_CANCEL}${session_id}`, {
                        method: 'PATCH',
                        credentials: 'include',
                        headers: {
    ...
    return (
        <div className="order-cancel-container">
            ...
            <p className="message success-message">Your order has been successfully canceled.</p>
            <p className="session-id">Session ID: <strong>{session_id}</strong></p>
